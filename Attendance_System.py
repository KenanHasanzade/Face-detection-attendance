...
if choice == 'MARK ATTENDANCE': 
    st.markdown("<h2 style='text-align: center; color: black;'>ATTENDANCE SYSTEM</h2>", unsafe_allow_html=True)
    ...
    if run:
        # Загружаем изображения из Register_Data
        images = []
        classNames = []
        for cl in myList:
        ...
        print("Registered names:", classNames)
        ...
        def findEncodings(images):
            ...
            for img in images:
                ...
                if encodes:  # проверка, что лицо найдено
        ...
        # RTSP или USB камера
        rtsp_url = "rtsp://admin:Admin123@172.30.42.242:554/Streaming/Channels/101"  # <-- сюда свою ссылку RTSP
        cap = cv2.VideoCapture(rtsp_url)  # можно заменить на 0 для USB камеры

        FRAME_WINDOW = st.image([])  # Streamlit окно для видео

        while True:
            success, img = cap.read()
            if not success or img is None:
                st.warning("Camera frame not available. Reconnecting...")
                time.sleep(1)
                continue  # пропустить итерацию, если кадр пустой

            # Уменьшение размера для ускорения работы
